...
void launch_gdn_forward_kv(const scalar_t* q,
  ...
  TORCH_CHECK(k_dim == 128 && v_dim == 128,
              "SM75 FlashQLA backend currently supports K=V=128");
...
void launch_gdn_decode_mixed_qkv_global_state_kv(
    ...
    cudaStream_t stream) {
  TORCH_CHECK(k_dim == 128 && v_dim == 128,
              "SM75 FlashQLA decode currently supports K=V=128");
...
void launch_gdn_decode_mixed_qkv_ddtree_state_kv(
    ...
    cudaStream_t stream) {
  TORCH_CHECK(k_dim == 128 && v_dim == 128,
              "SM75 FlashQLA DDTree decode currently supports K=V=128");
...
int get_column_groups_per_block(int tokens,
                                int q_heads,
                                int v_heads) {
  const char* raw = std::getenv("FLASH_QLA_SM75_COLUMN_GROUPS_PER_BLOCK");
  ...
  TORCH_CHECK(value == 1 || value == 2 || value == 4 || value == 8,
              "FLASH_QLA_SM75_COLUMN_GROUPS_PER_BLOCK must be one of 1, 2, 4, 8");
  return value;
}
...
int get_ddtree_column_groups_per_block(int tokens,
                                       int q_heads,
                                       int v_heads) {
  const char* raw = std::getenv("FLASH_QLA_SM75_DDTREE_COLUMN_GROUPS_PER_BLOCK");
  if (raw != nullptr && raw[0] != '\0') {
    const int value = std::atoi(raw);
    TORCH_CHECK(value == 1 || value == 2 || value == 4 || value == 8,
                "FLASH_QLA_SM75_DDTREE_COLUMN_GROUPS_PER_BLOCK must be one of "
                "1, 2, 4, 8");
    return value;
  }
...
std::vector<torch::Tensor> gdn_forward(torch::Tensor q,
  ...
  const auto stream = c10::cuda::getCurrentCUDAStream(q.device().index()).stream();
...
void gdn_decode_mixed_qkv_global_state(torch::Tensor mixed_qkv,
  ...
  const auto stream =
      c10::cuda::getCurrentCUDAStream(mixed_qkv.device().index()).stream();
