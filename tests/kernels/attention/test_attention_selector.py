...
def test_flashinfer_compute_capability_includes_sm75():
    backend = AttentionBackendEnum.FLASHINFER.get_class()

    assert backend.supports_compute_capability(DeviceCapability(7, 5))
    assert not backend.supports_compute_capability(DeviceCapability(7, 0))
